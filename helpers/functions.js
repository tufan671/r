...
let embed = new MessageEmbed().setColor('#0007ff').setTimestamp().setFooter(ayar.bot.footer)
let embed2 = new MessageEmbed().setColor('#da7cf0').setTimestamp().setFooter(ayar.bot.footer)
...
static async man(user, admin, name, channel) {
    ...
    if (nameMode && nameMode.nameMode === true) { channel.send(embed.setDescription(`${user} üyesinin ismi başarıyla \`"${name}"\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n${ayar.emojis.no} Kişinini toplamda **${data.length}** isim kayıtı bulundu. \n${isimler} \n\nKişinin önceki isimlerine \`.isimler @aspect/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)) } else { channel.send(embed.setDescription(`${user} üyesine ${user.guild.roles.cache.get(ayar.roles.manRoles[0])} rolü verildi.`)).sil(7); }
...
static async woman(user, admin, name, channel) {
    ...
    if (nameMode && nameMode.nameMode === true) { channel.send(embed2.setDescription(`${user} üyesinin ismi başarıyla \`"${name}"\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n${ayar.emojis.no} Kişinini toplamda **${data.length}** isim kayıtı bulundu. \n${isimler} \n\nKişinin önceki isimlerine \`.isimler @aspect/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)) } else { channel.send(embed2.setDescription(`${user} üyesine ${user.guild.roles.cache.get(ayar.roles.womanRoles[0])} rolü verildi.`)).sil(7); }
    if (ayar.guild.tagges.some(s => user.user.tag.toLowerCase().includes(s))) user.roles.add(ayar.roles.tagRole)
     if (ayar.guild.tagges.some(s => user.user.tag.toLowerCase().includes(s))) user.roles.add(ayar.roles.tagRole)
        }
