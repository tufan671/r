...
if(!data) new registerData({guildID: message.guild.id, nameMode: false}).save();
    switch (args[0]) {
        ...
        case "kapat":
            ...
            break;
        default:
            return message.channel.send(embed.setDescription(`İsim modu açık iken kayıt mesajında geçmiş isimler görüntülenir kapalı iken görüntülenmez \n \`\`\`${ayar.bot.botPrefix}${this.config.name} aç/kapat\`\`\` `)).sil(20);
            break;
    }
};
exports.config = {
    name: "isim-mod",
    guildOnly: true,
    aliases: ["isimmod","namemod","name-mod"],
    cooldown: 3000
};
