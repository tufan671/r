...
module.exports.run = async(client, message, args, embed) => {
    ...
    if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.member}, Kendine <@&${ayar.roles.tagRole}> rolü veremezsin!`)).sil(7)
    ...
    if (member.roles.cache.has(ayar.roles.tagRole)) {
        await member.roles.remove(ayar.roles.tagRole).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesinden <@&${ayar.roles.tagRole}> rolü alındı.`)).sil(7)

    } else {

        await member.roles.add(ayar.roles.tagRole).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesine <@&${ayar.roles.tagRole}> rolü verildi.`)).sil(7)
    }
